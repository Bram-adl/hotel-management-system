<template>
    <div>
                                ...
                                <table class="table table-hover">
                                    ...
                                    <tbody>
                                        <tr v-for="(customer, index) in customers.data" :key="customer.id">
                                            ...
                                            <td>
                                                <button class="btn btn-sm btn-success" @click="showEditModal(customer)"><i class="fas fa-edit"></i></button>
                                                <button class="btn btn-sm btn-danger" @click="deleteCustomer(customer.id)"><i class="fas fa-trash"></i></button>
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
        ...
        <modal
            :title="method == 'create' ? 'Create new customer' : 'Edit customer'"
            :submit="method == 'create' ? 'Create customer' : 'Update customer'"
    ...
    methods: {
        ...
        createCustomer: function () {
            ...
            this.form.post("/api/customers")
                .then(response => {
                    ...
                    this.fireToast("success", "Customer created successfully!")
                    this.closeModal()
                    this.fetchCustomers()
                })
                .catch(error => {
        ...
        updateCustomer: function () {
            ...
            this.form.put(`/api/customers/${this.form.id}`)
                .then(response => {
                    this.$Progress.finish()
                    this.fireToast("success", "Customer updated successfully!")
                    this.closeModal()
                    this.fetchCustomers()
                })
                .catch(error => {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    axios.delete(`/api/customers/${id}`)
                        .then(response => {
                            this.$Progress.finish()
                            this.fireSwal("success", "Deleted!", "Customer deleted successfully!")
                            this.fetchCustomers()
                        })
                        .catch(error => {
                            this.$Progress.fail()
